...
- **Deck pickup** (both v1 `app.ts` and v2 `deck/`): after a qualifying save the deck polls `GET /api/generations?response_id=…` (`src/lib/deck-order.ts` owns the schedule, splice math, boot re-ordering that places AI cards after their parent, and the trigger check). Complete-screen edge pulls the respondent back to the follow-up. v1 deliberately patches progress chrome in place instead of `draw()` — a full re-render wipes in-progress input on other cards.
