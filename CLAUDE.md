...
- **Three gates, all default-off, re-read fresh per generation**: env (`ANTHROPIC_API_KEY` or `REACTIVE_FAKE_MODE`, + `REACTIVE_CARDS_ENABLED`) → `organizations.reactive_cards_allowed` (superadmin PATCH + toggle on `/admin/#superadmin`) → `engagements.reactive_cards_enabled` (edit-engagement dialog; 403 via `reactive.ensure_org_allowed` only on the false→true transition, shared with MCP `pulse_update_engagement`).
- **Scheduling — the load-bearing constraint**: `get_anon_session` holds the request's OUTER connection transaction until dependency teardown (the GUCs are `set_config(..., is_local=true)`), and background work runs BEFORE that teardown on this fastapi/starlette pairing. So `save_response` extracts the trigger text from the request body it already validated and calls `reactive.schedule_generation` — a detached `asyncio.create_task` with a strong-reference registry — never FastAPI `BackgroundTasks` (a task that waits for its own request's commit deadlocks: the commit waits for the task). The engine's context-load retry loop absorbs the few-ms commit-visibility window. **Apply the same pattern to any future post-request work in client routes.**
- **Engine phases** (all on the BYPASSRLS `admin_engine`; `pulse_anon`/`pulse_member` have SELECT-only on `card_generations`): gates + per-recipient cap (counts ATTEMPTS via `card_generations`, not created cards — cost bound) + the duplicate-follow-up guard (`cards_repo.has_unanswered_ai_followup` — an unanswered AI card already generated from THIS `response_id` skips a repeat generation outright; `responses` upserts on `(card_id, recipient_id)`, so re-editing the same card's correction reuses the same `response_id` and would otherwise dodge the dedup claim below) + dedup claim on unique `(response_id, trigger_hash)` committed BEFORE the LLM call → deck-listing read (session closed) → Anthropic call with NO session open (shared pool protection) → outcome write + `record_audit("card.reactive_generate", user_id=None)`. Failures mark the row `failed` and never surface to the respondent.
- **LLM call**: `anthropic` SDK (`AsyncAnthropic`), structured output via `output_config={"format": {"type": "json_schema", ...}}`, `stop_reason` checked before content (`refusal` → skipped). Respondent text is fenced (`<respondent_correction>`, embedded tags neutralized). `validate_proposals` is the REAL trust boundary: type whitelist (select/text only), length clamps, `attachment_path`/`default_value` forced None, `skip_allowed` forced True. Token usage + `cost_usd` (from `MODEL_PRICING`) recorded on every generation row — the superadmin usage report (`GET /api/superadmin/reactive-usage`, panel on both superadmin UIs) aggregates them per org.
- **Deck pickup** (both v1 `app.ts` and v2 `deck/`): a qualifying save does NOT advance — the deck stays on the corrected card in a "waiting" mode (inputs/actions not rendered) and polls `GET /api/generations?response_id=…` on a tight ~8s budget (`waitSchedule`). A completion inside that budget splices the new card(s) in after the corrected card and navigates straight into the first one; skipped/failed/budget-expiry all fall through to a normal `advance()`, and ONLY the budget-expiry path then falls back to a background poll on the original ~40s `pollSchedule` so a late completion still splices in behind the respondent. `src/lib/deck-order.ts` owns both schedules, `runPoll` (schedule is now a parameter), splice math, boot re-ordering that places AI cards after their parent, and the trigger check. User-driven navigation (nav-back/forward, picker jump) cancels an in-flight wait and is always honored. v1 deliberately patches progress chrome in place instead of `draw()` for the background-fallback case — a full re-render wipes in-progress input on other cards.
- **Dev fake mode**: `REACTIVE_FAKE_MODE=true` = canned 2-card proposal, no key, zero outbound HTTP, every gate/cap/validation still exercised. Note `docker compose restart` does NOT re-read `.env` — use `docker compose up -d backend`.
- **Tests**: `tests/unit/test_reactive_unit.py` (pure functions), `tests/test_reactive_cards.py` (engine + routes, respx-mocked `api.anthropic.com`; await `reactive.wait_for_pending_generations()` after HTTP-driven saves), RLS isolation cases in `test_rls_isolation.py`. The commit-ordering behavior above is INVISIBLE to the suite (conftest replaces the session plumbing) — verify scheduling changes against the live stack.
...
Production runs on a **shared Debian VPS** (other apps coexist on the same host). Ansible playbook in `deploy/`; the standard entry point is `./deploy.sh` at the repo root — the same script shape used by the other Axiolo deploys (`sitechecker`, `octoping`). It reads the vault password from `deploy/vault_secret`, prompts for the VPS sudo password (or takes `BECOME_PASSWORD`), warns if the local branch differs from origin, and supports `--check` / `--branch` / `--tags` / `--limit` (`--help` for the full list). `make deploy-check` / `make deploy-apply` delegate to it. See `deploy/README.md` for the full runbook.

**Critical constraint, baked into every role:** Ansible only performs narrow shared-host setup: install missing apt packages with `state: present` (no upgrades, no third-party repositories — `nodejs` + `npm` come from Debian apt, not NodeSource), and start/enable Postgres + nginx. The other roles (`postgres-pulse`, `backend`, `frontend`, `nginx-site`, `tls`) are scoped to their own paths/units/DB/roles only. First-run command is always `./deploy.sh --check` (wraps `ansible-playbook … --check --diff`) so the operator can read every diff before applying.
