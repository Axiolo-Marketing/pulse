...
### AI follow-up cards (reactive)

Cards carry three provenance/scoping columns beyond the authoring fields above:

- **recipient_id** (nullable) — `NULL` means the card is shared by every recipient of the engagement (all operator-authored cards). When set, only that recipient sees the card in their deck; other recipients' RLS reads exclude it.
- **source** — `'operator'` (default) or `'ai'`. AI cards are generated by the reactive-cards engine (§15) and badged in admin.
- **generated_from_response_id** (nullable) — back-link to the response whose correction triggered the generation; the deck uses it to place the follow-up directly after its parent card.

Operators can edit and delete AI cards exactly like their own. Engagement reset removes them. See §15 for the full feature.
...
As of migration 0017 (reactive cards, §15), `cards` additionally carries
`recipient_id uuid` (nullable FK → recipients, on delete cascade; NULL =
engagement-shared), `source text` (`'operator'`/`'ai'`), and
`generated_from_response_id uuid` (nullable FK → responses, on delete set
null). The `cards_self_read` RLS policy narrows to
`recipient_id IS NULL OR recipient_id = pulse_request_recipient_id()`.

**card_generations** — reactive-generation lifecycle record, dedup lock, and
per-org LLM cost ledger (migration 0017)
| Column | Type | Notes |
|---|---|---|
| id | uuid PK | |
| org_id | uuid FK → organizations | on delete cascade; the cost-report grouping key |
...
| **unique** | (response_id, trigger_hash) | idempotency claim — identical re-saves dedup |

Grants: SELECT-only for `pulse_anon` (own-recipient rows, for deck polling)
and `pulse_member` (org rows); only `pulse_admin` (the generation engine)
writes.
...
Reactive cards (§15; all default to off/safe):
...
| v4 | Reactive cards: LLM follow-ups on corrections, per-recipient card scoping, per-org cost ledger (§15) | branch `reactive-cards` |
...
## 15. Reactive cards (LLM follow-up questions) (v4)

When a respondent corrects a pre-populated statement ("Needs edit" on a
`confirm-edit` card + free-text correction), Pulse can react in the moment:
an LLM evaluates the correction against the engagement context and — only
when the correction is materially ambiguous, incomplete, or contradictory —
inserts up to 2 follow-up question cards into **that respondent's** deck,
live, while they are still in the session. The consultant gets clarification
without a second engagement round-trip.

### 15.1 Trigger surface (deliberately narrow)

Only a `confirm-edit` card saved as `answered` with
`{confirmed: false, correction: "<non-empty text>"}` triggers evaluation.
Notes, free-text answers, skips, and `confirmed: true` never do. Answers to
AI-generated cards never re-trigger (depth 1 — no chains).

### 15.2 Three gates, all default-off

1. **Deployment** — `ANTHROPIC_API_KEY` set (or `REACTIVE_FAKE_MODE`) and
   `REACTIVE_CARDS_ENABLED=true`.
2. **Organization** — `organizations.reactive_cards_allowed`, flipped only by
   a superadmin (toggle on `/admin/#superadmin`, or
   `PATCH /api/superadmin/orgs/{id}`).
3. **Engagement** — `engagements.reactive_cards_enabled`, flipped by org
   members in the edit-engagement dialog (403 unless the org is allowed;
   only the false→true transition is gated, so an org-level revocation never
   breaks edits to an already-enabled engagement).

The engine re-reads gates 2 and 3 fresh on every generation, so a revoked
org stops generating immediately.
...
### 15.4 Generation pipeline (backend)

`POST /api/responses` extracts the correction, then schedules
`reactive.run_generation` as a detached asyncio task (never FastAPI
`BackgroundTasks` — see CLAUDE.md for the transaction-ordering constraint).
The engine, on the BYPASSRLS admin engine:
...
The model is told the correction is untrusted data (fenced, with embedded
fence tags neutralized) and to *prefer proposing nothing* — most corrections
stand on their own.

### 15.5 Operator experience

- **Engagement detail** (both admin UIs): AI cards carry an
  "AI follow-up · for <recipient>" badge with the triggering card in the
  tooltip. Edit/delete work like any card.
- **Activity feed**: "AI follow-up generated" entries.
- **Superadmin** (`/admin/#superadmin`): per-org allow toggle, plus a
  "Reactive cards usage" panel — calls by status, token totals, and
  estimated cost per org over a 30/90-day window
  (`GET /api/superadmin/reactive-usage?days=N`). Monitoring only, not a
  billing surface. Costs are stamped per call from a model price map, so
...
`REACTIVE_FAKE_MODE=true` waives the API key and short-circuits the LLM call
with a canned 2-card proposal (model `fake`, $0, zero outbound HTTP) while
still exercising every gate, cap, dedup, validation, and UI path — the whole
loop is clickable locally without credentials.
