...
@mcp.tool(
    ...
    description=(
        ...
        "Cards also carry `source` (`\"operator\"` or `\"ai\"`) and "
        "`recipient_id`: an `\"ai\"` card was auto-generated by the "
        "reactive-cards engine from a respondent's correction, is only "
        "visible to the `recipient_id` it's scoped to (`null` "
        "`recipient_id` means the card is shared across every "
        "recipient), and links back to the triggering response via "
...
@mcp.tool(
    ...
    description=(
        ...
        "engagement. Set `reactive_cards_enabled` to turn the reactive-"
        "cards engine (auto-generated AI follow-up questions on respondent "
        "corrections) on or off for this engagement — turning it on fails "
        "with an error unless the org has reactive cards allowed "
        "(superadmin-managed)."
    ),
)
async def pulse_update_engagement(
    ...
    if reactive_cards_enabled is not None:
        fields["reactive_cards_enabled"] = reactive_cards_enabled

    async with _open_member_session(org_id) as session:
        if fields.get("reactive_cards_enabled") is True:
            # Same false→true-only gate as the REST route: resending an
            # already-enabled flag never 403s after an org revocation.
